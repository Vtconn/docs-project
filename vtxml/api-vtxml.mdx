---
title: 'API do VTXML'
description: 'API para poder salvar NFe e CTe via REST'
---

A API está configurada para receber os dados pelo mesmo endpoint, a diferença será feita pelo campo `tipo` no body da requisição, e dependo do body será necessário enviar diferente json na requisição

Rota da API [POST]: http://191.252.197.120:8990/rest/restvtxml/saveDocument

## Salvar uma CTe

Para salvar uma CTe é necessário enviar as informações de cabeçalho e de um único item esse item é buscado através de uma função do ADVPL chamada `U_CP01005G("11", "PRODTRANSP")`

CFOP deve ser salvo como string.

Dados necessários para enviar no body da requisição:

```json
{
  "tipo": "cte",
  "chaveNota": "text",
  "numeroNota": "text",
  "serieNota": "text",
  "dataNota": "dd/mm/yyyy",
  "tipoNota": "text",
  "tipoPE": "text",
  "naturezaOperacao": "text",
  "cnpj": "text",
  "razaoSocial": "text",
  "uf": "text",
  "valorTotal": 0000,
  "notaValida": "text",
  "status": "text",
  "codigoEmpresa": "text",
  "codigoFilial": "text",
  "chaveReferencia": "text",
  "dataImpressao": "dd/mm/yyyy",
  "horarioImpressao": "text",
  "usuarioImpressao": "text",
  "codigoAdd": "text",
  "valorFrete": 0000,
  "items": [
    {
      "cfop": "0000"
    }
  ]
}
```

Caso salve a CTe com sucesso, é retornado o seguinte json:

```json
{
  "success": true,
  "message": "CTe salvo com sucesso, chave da nota: numero da chave"
}
```

Caso já tenha sido salvo o documento, será retoranado o status 400 e o seguinte json:

```json
{
  "errorCode": 400,
  "errorMessage": "CTE ja importado anteriormente"
}
```

Outros erros serão retornados o status 500

## Salvar uma NFe

Para salvar uma NFe é necessário enviar as informações de cabeçalho e

Dados necessários para enviar no body da requisição:

```json
{
  "tipo": "nfe",
  "chaveNota": "text",
}
```

Caso salve a NFe com sucesso, é retornado o seguinte json:

```json
{
  "success": true,
  "message": "NFe salvo com sucesso, chave da nota: numero da chave"
}
```

Caso já tenha sido salvo o documento, será retoranado o status 400 e o seguinte json:

```json
{
  "errorCode": 400,
  "errorMessage": "NFe ja importado anteriormente"
}
```

Outros erros serão retornados o status 500

## Calculo dos impostos

Aqui estará documento como está sendo o feito do calculo dos impostos:

### Relação da Nota e dos impostos

Aqui está a a lista dos impostos na DANFE e no json:

| Imposto | DANFE | JSON | Tipo |
| ------- | ----- | ---- | ---- |
| ICMS | ORIG | icmsOrig | String |
| ICMS | CST | icmsCst | String |
| ICMS | VBC | icmsBc | Number |
| ICMS | PICMS | icmsAliquota | Number |
| ICMS | VICMS | icmsValor | Number |
| ICMS | VBCST | icmsSTBC | Number |
| ICMS | PICMSST | icmsSTAL | Number |
| ICMS | VICMSST | icmsSTVL | Number |
| IPI | IPI IPITRIB CST | ipiCst | String |
| IPI | VBC | ipiBC | Number |
| IPI | VIPI | ipiValor | Number |
| IPI | PIPI | ipiAliquota | Number |
| PIS | PIS CST | pisCst | Number |
| PIS | PPIS | pisAliquota | Number |
| PIS | VPIS | pisValor | Number |
| PIS | COFINS CST | cofinsCst | String |
| PIS | VBC and VCOFINS and PCOFINS - VBC | cofinsBC | Number |
| PIS | VBC and VCOFINS and PCOFINS - PCOFINS | cofinsAliquota | Number |
| PIS | VBC and VCOFINS and PCOFINS - VCOFINS | cofinsValor | Number |

### Impostos para CTe

Segue a relação dos impostos no json para CTe:

```md
**CST (Código de Situação Tributária):**
data.xml.CTe.infCte.imp.ICMS.ICMS00.CST

**Base de Cálculo (BC):**
data.xml.CTe.infCte.imp.ICMS.ICMS00.vBC

**Alíquota (%):**
data.xml.CTe.infCte.imp.ICMS.ICMS00.pICMS

**Valor do ICMS:**
data.xml.CTe.infCte.imp.ICMS.ICMS00.vICMS
```

## Status code retorno da API

| Status | Descrição |
| ------ | --------- |
| 201 | Nota salva com sucesso |
| 400 | Quando envia um tipo incorreto diferente de `cte` ou `nfe` |
| 409 | Nota já importada anteriormente |
| 500 | Erro interno |